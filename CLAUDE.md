...
- **`packages/solver`** — the rebalancing engine. Pure TypeScript: no DOM, no `fetch`, no filesystem access, no network calls. Deterministic — same input always produces the same output. Its public exports are `rebalance()`, `validateScenario()`, `DEFAULT_TOLERANCE_BPS`, and the domain types in `src/types.ts`; nothing else in `src/` should be imported from outside the package.
  ...
  - `src/allocate.ts` — the optimizer seam. `allocate(problem)` works purely in (account × asset class) space — ids and integer cents; no funds, no names. Money never leaves an account, so each account's post-trade total is fixed (a fixed-supply transportation problem). Do not hand-roll a min-cost-flow solver.
  - `src/allocate.lp.ts` — the LP-backed alternative (`options.optimizer: "lp"`; greedy stays the default), using YALPS (pure JS, synchronous — the async/WASM LP libraries would break `rebalance()`'s sync purity). Lexicographic objectives (deviation → total sells → taxable sells → tax-preferred placement), each pinned as a constraint before the next solve. Simplex floats are rounded back to integer cents by a per-account largest-remainder repair: account totals stay exact; class totals can carry < 1 cent of noise per account, which the property tests account for as explicit slack. The tolerance band is an *eligibility* test computed from the inputs (same as greedy): beyond-band classes are pulled to their exact target, within-band classes are frozen against selling. `minTradeCents` is combinatorial and not modeled — lp mode ignores it with a warning. `src/allocate.property.test.ts` brute-forces small problems to hold *both* implementations to the same optimality bar, plus a differential property (lp never worse than greedy).
